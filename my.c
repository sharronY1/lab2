...
int handle_label_or_imm(int line_no, char *val, struct_label_table *label_table, int number_of_labels) {
    ...
    if ((ret = is_imm(val)) == MISMATCH) {
        ...
        if (is_imm_in_range(_val, 20))   // 20 digit at most
            return _val;
        else
...
struct_regs_indirect_addr* parse_regs_indirect_addr(char *arg2, int line_no) {
    ...
    temp[i] = '\0';
    imm = strtok(temp, "( ");    //8(t0), get number before '('
    ret->imm = validate_imm(imm, 12, line_no);
    reg = strtok(NULL, ")");    // get t0 before )
    ...
    */
    ...
    sprintf(  // print to char temp[]
        ...
        (handle_label_or_imm(
            ...
            label_table,
            number_of_labels
        )) & 0xFFF    // only lower 12 bits are preserved
    );
    // addi
    fprintf(
        output_file,
...
int inst_to_binary(
    ...
    if (is_opcode(opcode) == ADDI) {
        ...
        binary += (reg_to_num(arg1, line_no) << 7);
        binary += (reg_to_num(arg2, line_no) << 15);
        int shift_imm = lower5bit(arg3, line_no);
        binary += shift_imm << 20;
        binary += ((0x00 << 3) + 0x00) << 24;
